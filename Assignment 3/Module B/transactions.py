...
def atomic_mark_attendance(member_id, schedule_id, status, amount):
    ...
    cur = conn.cursor()
    try:
        log_id = _next_id(cur, 'MealLog', 'LogID')
        cur.execute("""
            INSERT INTO MealLog (LogID, MemberID, ScheduleID, Status)
            VALUES (%s, %s, %s, %s)
            ON DUPLICATE KEY UPDATE Status = VALUES(Status)
        """, (log_id, member_id, schedule_id, status))

        if status == 'Consumed':
            meal_pay_id = _next_id(cur, 'MealPayment', 'MealPaymentID')
            cur.execute("""
                INSERT INTO MealPayment (MealPaymentID, MemberID, ScheduleID, Amount, PaymentDate)
...
def atomic_update_billing_status(payment_id, new_status, username):
    ...
    try:
        ...
        cur.execute("""
            UPDATE MonthlyMessPayment
            SET Status = %s
            WHERE MonthlyPaymentID = %s
